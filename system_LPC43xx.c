 ...
 *----------------------------------------------------------------------------*/
    ...
    if (PLL0USB_M > 16384)
	seli = 1;
    else if (PLL0USB_M > 8192)
	seli = 2;
    else if (PLL0USB_M > 2048)
	seli = 4;
    else if (PLL0USB_M >= 501)
	seli = 8;
    else if (PLL0USB_M >= 60)
	seli = 4 * (1024 / (PLL0USB_M + 9));
    else
	seli = (PLL0USB_M & 0x3C) + 4;
...
void SystemInit_ExtMemCtl(void)
{
    ...
    //  LPC_EMC->STATICCONFIG0  = (1 <<  7) | /* Byte lane state: use WE signal    1 */
    //                            (2 <<  0) | /* Memory width 32-bit                */
    ////                            (0 <<  0) | /* Memory width 16-bit                */
    //                            (1 <<  3);  /* Async page mode enable         1    */

    //  LPC_EMC->STATICWAITOEN0 = (2 <<  0) ; /* Wait output enable: No delay       0*/

    //  LPC_EMC->STATICWAITPAGE0 = 2;

    //  /* Set Static Memory Read Delay for 90ns External NOR Flash                 */
    //  LPC_EMC->STATICWAITRD0  = 0x015 ; //1 + EMC_NANOSEC(90, SystemCoreClock, div);
    //  LPC_EMC->STATICCONFIG0 |= (0 << 19) ; /* Enable buffer                      1*/
    //#endif
